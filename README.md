[代码实现详解.md](https://github.com/user-attachments/files/23622443/default.md)
# 代码实现详解 - 如何写成这些功能

## 1️⃣ 对话框拖拽功能的完整实现

### 核心原理
```
用户按下鼠标 → 记录初始位置 → 移动鼠标 → 计算移动距离 → 更新对话框位置 → 松开鼠标 → 停止拖拽
```

### 完整代码示例

```javascript
// 第1步：创建对话框
const dialog = document.createElement('div');
dialog.style.cssText = `
    position: fixed;
    top: 50%;
    left: 50%;
    transform: translate(-50%, -50%);
    z-index: 10000;
    width: 800px;
    height: 600px;
`;

// 第2步：初始化位置（重要！）
const initialRect = dialog.getBoundingClientRect();
dialog.style.left = initialRect.left + 'px';
dialog.style.top = initialRect.top + 'px';
dialog.style.transform = 'none';  // 清除transform

// 第3步：定义拖拽变量
let isDragging = false;
let startX = 0, startY = 0;
let elementX = 0, elementY = 0;

// 第4步：鼠标按下事件
dialogHeader.addEventListener('mousedown', (e) => {
    // 检查是否点击关闭按钮
    if (e.target.classList.contains('dialog-close')) {
        return;  // 不启动拖拽
    }
    
    isDragging = true;
    startX = e.clientX;      // 记录鼠标X坐标
    startY = e.clientY;      // 记录鼠标Y坐标
    
    const rect = dialog.getBoundingClientRect();
    elementX = rect.left;    // 记录对话框左边距离
    elementY = rect.top;     // 记录对话框顶部距离
    
    dialogHeader.style.cursor = 'grabbing';
    e.preventDefault();
});

// 第5步：鼠标移动事件
const dragMove = (e) => {
    if (!isDragging) return;
    
    // 计算鼠标移动距离
    const deltaX = e.clientX - startX;
    const deltaY = e.clientY - startY;
    
    // 计算新位置
    const newX = elementX + deltaX;
    const newY = elementY + deltaY;
    
    // 限制在屏幕范围内
    const maxX = window.innerWidth - dialog.offsetWidth;
    const maxY = window.innerHeight - dialog.offsetHeight;
    
    const constrainedX = Math.max(0, Math.min(newX, maxX));
    const constrainedY = Math.max(0, Math.min(newY, maxY));
    
    // 更新对话框位置
    dialog.style.left = constrainedX + 'px';
    dialog.style.top = constrainedY + 'px';
};

// 第6步：鼠标松开事件
const dragEnd = () => {
    isDragging = false;
    dialogHeader.style.cursor = 'move';
};

// 第7步：注册事件监听
dialogHeader.addEventListener('mousedown', () => {
    document.addEventListener('mousemove', dragMove);
    document.addEventListener('mouseup', dragEnd);
});

// 第8步：关闭时清理事件
dialog.querySelector('.dialog-close').addEventListener('click', () => {
    document.removeEventListener('mousemove', dragMove);
    document.removeEventListener('mouseup', dragEnd);
    document.body.removeChild(dialog);
});
```

### 关键点说明

1. **为什么要清除transform？**
   - 初始化时用 `transform: translate(-50%, -50%)` 居中
   - 拖拽时需要用 `left/top` 定位
   - 两种方式混用会导致坐标混乱

2. **为什么要记录初始位置？**
   - `getBoundingClientRect()` 获取实际像素位置
   - 用于计算相对移动距离

3. **为什么要限制在屏幕范围内？**
   - 防止对话框拖出屏幕外
   - 提升用户体验

---

## 2️⃣ 数据分配器分类的实现

### HTML结构

```html
<!-- 基础结构 -->
<li>
    <details>
        <summary>数据分配器</summary>
        <ul style="margin-left: 20px;">
            <li data-type="74ls138">74LS138 1路-8路</li>
            <li data-type="74139">74LS139 1路-4路</li>
        </ul>
    </details>
</li>
```

### 如何添加新分类

```html
<!-- 示例：添加"计数器"分类 -->
<li>
    <details>
        <summary>计数器</summary>
        <ul style="margin-left: 20px;">
            <li data-type="74ls160">74LS160 同步计数器</li>
            <li data-type="74ls161">74LS161 同步计数器</li>
            <li data-type="mod-n-counter">模N计数器</li>
        </ul>
    </details>
</li>
```

### JavaScript处理

```javascript
// 点击时自动展开/折叠
document.querySelectorAll('details').forEach(detail => {
    detail.addEventListener('click', (e) => {
        // details元素自动处理展开/折叠
        // 无需额外代码
    });
});

// 点击子项时创建组件
document.querySelectorAll('li[data-type]').forEach(li => {
    li.addEventListener('click', (e) => {
        const type = li.dataset.type;
        createComponentFromSidebar(type);
    });
});
```

---

## 3️⃣ 蓝色选中方框的实现

### 核心代码

```javascript
// 选择组件
function selectComponent(component) {
    selectedComponents.add(component);
    
    // 设置蓝色边框
    component.style.outline = '2px solid #007bff';
    
    // 边框向内缩进20px
    component.style.outlineOffset = '-20px';
}

// 取消选择
function deselectComponent(component) {
    selectedComponents.delete(component);
    component.style.outline = '';
    component.style.outlineOffset = '';
}
```

### 属性详解

```javascript
// outline: 边框样式
// 格式: [宽度] [样式] [颜色]
component.style.outline = '2px solid #007bff';
//                         ↑    ↑     ↑
//                        宽度  样式  颜色

// outlineOffset: 边框距离
// 正值：向外扩展
// 负值：向内收缩
component.style.outlineOffset = '-20px';
//                               ↑
//                          向内缩进20px
```

### 自定义方框大小

```javascript
// 方案1：改变边框宽度
component.style.outline = '3px solid #007bff';  // 更粗

// 方案2：改变内缩距离
component.style.outlineOffset = '-25px';  // 更紧贴

// 方案3：改变颜色
component.style.outline = '2px solid #FF0000';  // 红色

// 方案4：组合修改
component.style.outline = '3px solid #00FF00';
component.style.outlineOffset = '-15px';
```

---

## 4️⃣ 完整的新实验创建流程

### 步骤1：创建SVG组件文件 (新组件.js)

```javascript
// 定义SVG模板
const newComponentSvg = {
    'new-component': `
        <svg class="component-svg" viewBox="0 0 200 200">
            <rect x="50" y="50" width="100" height="100" fill="#E8F4FD"/>
            <text x="100" y="105" text-anchor="middle">新组件</text>
        </svg>
    `
};

// 定义逻辑函数
const newComponentLogic = {
    'new-component': (inputs) => {
        // 处理输入，返回输出
        return { output: inputs.input };
    }
};

// 初始化函数
function initializeNewComponent(componentDiv, type) {
    componentDiv.title = '新组件 - 双击查看说明';
    componentDiv.addEventListener('dblclick', showNewComponentDialog);
}

// 对话框函数
function showNewComponentDialog() {
    const dialog = document.createElement('div');
    dialog.style.cssText = `
        position: fixed;
        top: 50%;
        left: 50%;
        transform: translate(-50%, -50%);
        z-index: 10000;
        width: 800px;
        height: 600px;
    `;
    
    dialog.innerHTML = `
        <div style="width: 100%; height: 100%; background: white; 
                    border: 2px solid #333; display: flex; flex-direction: column;">
            <div style="background: #2196F3; color: white; padding: 15px 20px; 
                        cursor: move; display: flex; justify-content: space-between;">
                <h2>新组件说明</h2>
                <button onclick="this.closest('div').parentElement.remove()">×</button>
            </div>
            <div style="padding: 20px; overflow-y: auto; flex: 1;">
                <h3>功能说明</h3>
                <p>这是一个新组件的说明...</p>
            </div>
        </div>
    `;
    
    document.body.appendChild(dialog);
    // 添加拖拽功能...
}

// 暴露到全局
window.newComponent = {
    initialize: initializeNewComponent,
    svg: newComponentSvg,
    logic: newComponentLogic
};
```

### 步骤2：在merged_complete.html中引入

```html
<!-- 在<head>中添加 -->
<script src="新组件.js"></script>
```

### 步骤3：在实验列表中添加

```html
<!-- 在第515-523行的数据分配器后添加 -->
<li data-type="new-component">新组件名称</li>
```

### 步骤4：注册组件

```javascript
// 在merged_complete.html的初始化代码中添加
if (window.newComponent && window.newComponent.svg) {
    componentSvgs['new-component'] = window.newComponent.svg['new-component'];
    logicFunctions['new-component'] = window.newComponent.logic['new-component'];
}
```

---

## 📝 总结

| 功能 | 核心代码 | 关键点 |
|------|---------|--------|
| 拖拽 | `getBoundingClientRect()` + `left/top` | 清除transform |
| 分类 | `<details>` + `<ul>` | data-type属性 |
| 方框 | `outline` + `outlineOffset` | 负值向内缩 |
| 新组件 | SVG + 逻辑函数 + 初始化 | 全局注册 |

